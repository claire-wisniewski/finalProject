...
def main():
    ...
    parser.add_argument("--n-trees", type=int, default=50, help="Number of custom ensemble trees")
    parser.add_argument("--baseline-n-trees", type=int, default=50, help="Number of baseline RF trees")
    ...
    parser.add_argument("--noise-model", type=str, default="gaussian", choices=["gaussian", "laplace", "poisson"])
    ...
    results_df.to_csv(f"{stem}_prediction_results.csv", index=False)
    feature_df = clf.get_feature_importance_table()
    feature_df.to_csv(f"{stem}_feature_sensitivity.csv", index=False)
    ...
    print(f"\nSaved prediction results to: {stem}_prediction_results.csv")
    print(f"Saved feature sensitivity table to: {stem}_feature_sensitivity.csv")
